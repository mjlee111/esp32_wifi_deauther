...
bool WiFi_Utils::connectToWifi(std::string SSID, std::string PASSWORD,
...
std::vector<int> WiFi_Utils::scanPortsInRange(IPAddress ip, int startPort,
                                              int endPort, int timeout_ms) {
...
void WiFi_Utils::sendDeauthPacket(const uint8_t *bssid, const uint8_t *sta) {
  ...
  esp_wifi_80211_tx(WIFI_IF_STA, (uint8_t *)&deauth_frame, sizeof(deauth_frame),
                    true);
}

std::string WiFi_Utils::macAddress() { return mac_address; }